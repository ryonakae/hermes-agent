...
def _sdk_http_error_type() -> type[Exception]:
    """Return the HTTP error base class used by the installed MCP SDK."""
    from tools.mcp_tool import sdk_httpx

    httpx = sdk_httpx()
    if httpx is None:  # pragma: no cover - MCP OAuth cannot run without its transport
        raise RuntimeError("MCP SDK HTTP transport is unavailable")
    ...
    class _HermesOAuthClientProvider(OAuthClientProvider):
        ...
        async def _handle_token_response(self, response):
            ...
            if 200 <= response.status_code < 300:
                ...
                HTTPError = _sdk_http_error_type()
